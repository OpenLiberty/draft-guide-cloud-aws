...
cloud-based infrastructure allows you to focus on developing your microservices rather 
than worrying about details that are related to the servers you deploy them to. Using 
a cloud allows you to easily scale your microservices and serve them in a high-availability 
setup.

Amazon Web Services (AWS) offers a managed Kubernetes service called Amazon EKS. EKS 
simplifies the process of running Kubernetes on AWS without needing to install or maintain 
your own Kubernetes control plane. It provides a hosted {kube} cluster that you can deploy 
your microservices to. You will use it in conjunction with Amazon Elastic Container 
Registry (ECR). Amazon ECR is a private registry used to store and distribute your
container images.
...
The `[aws_account_id]` is a unique 12-digit ID that is assigned to every AWS account. Throughout
the guide, you will notice this ID come up in the output from various commands since AWS uses
it to differentiate your resources from other accounts. 
...
Take note of the repository URI for both the `system` and `inventory` repositories as you will need 
...
Before you can make a request to `EXTERNAL-IP:31000` or `EXTERNAL-IP:32000`, you must modify
